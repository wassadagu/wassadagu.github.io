#!/usr/bin/env python3
"""Build the public hub index from per-media mapping documents."""
import json
import re
...
legacy = {item.get("media_id"): item for item in document.get("items", []) if item.get("media_id")}
items = {}
for path in sorted((root / "links").glob("*.json")):
    ...
    if not media_id or not item.get("sharelink_url"):
        continue
    merged = dict(legacy.get(media_id, {}))
    merged.update({key: value for key, value in item.items() if value not in (None, "")})
    merged["media_id"] = media_id
    items[media_id] = merged
# Preserve old index entries until their per-media file is migrated.
for media_id, item in legacy.items():
    items.setdefault(media_id, item)
